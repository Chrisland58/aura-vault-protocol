...
# ── Issue #516: RDS Read Replica for Analytics Queries ──────────────────────

variable "enable_read_replica" {
  description = "Provision a read replica for analytics queries"
  type        = bool
  default     = true
}
...
resource "aws_db_instance" "replica" {
  ...
  # Source the primary instance
  replicate_source_db = aws_db_instance.main.identifier

  instance_class = var.replica_instance_class

  # Replicas inherit storage settings from the source; these can be overridden
  storage_encrypted = true

  # Do NOT set multi_az on the replica itself — it is already a standby
  multi_az = false

  # Replicas use the same subnet group and SG as primary
  db_subnet_group_name   = aws_db_subnet_group.main.name
  vpc_security_group_ids = [aws_security_group.rds.id]

  publicly_accessible = false

  # Performance Insights on the replica
  performance_insights_enabled = true
  monitoring_interval          = 60
  monitoring_role_arn          = aws_iam_role.rds_monitoring.arn

  # Allow promotion to primary in < 2 minutes (automatic minor version upgrades
  # and maintenance windows are synced with primary to minimise promotion lag)
  auto_minor_version_upgrade = true

  # Replicas cannot be deleted-protected independently; protect via primary
  deletion_protection = false
  skip_final_snapshot = true

  tags = {
    Name = "${var.project_name}-db-replica-${var.environment}"
    Role = "analytics-read-replica"
  }

  depends_on = [aws_iam_role_policy_attachment.rds_monitoring]
}

# CloudWatch alarm: replication lag > 30 seconds
resource "aws_cloudwatch_metric_alarm" "replication_lag" {
  count               = var.enable_read_replica ? 1 : 0
  alarm_name          = "${var.project_name}-replication-lag-${var.environment}"
  ...
  statistic           = "Average"
  threshold           = 30
  alarm_description   = "RDS read replica lag exceeded 30 seconds. Analytics queries may be stale."
  ...
  tags = {
    Name = "${var.project_name}-replication-lag-${var.environment}"
  }
}
