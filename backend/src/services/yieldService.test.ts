...
import {
  startYieldScheduler,
  stopYieldScheduler,
  isYieldSchedulerRunning,
  msUntilNextHour,
  getSchedulerMetrics,
} from "./yieldScheduler.js";
...
const stakingSource: YieldSource = { type: "staking", apy: 0.08 };    // 8% APY
const feesSource: YieldSource = { type: "fees", apy: 0.04 };          // 4% APY
const incentivesSource: YieldSource = { type: "incentives", apy: 0.02 }; // 2%
...
describe("totalCompoundYield", () => {
  ...
  it("returns 0 when calcDate is before entryDate", () => {
    const result = totalCompoundYield(
      1_000,
      [stakingSource],
      new Date("2025-01-10"),
      new Date("2025-01-01") // before entry
    );
    expect(result).toBe(0);
  });
...
describe("YieldService.calculateForPosition", () => {
  ...
  it("handles all three yield source types in one calculation", () => {
    ...
    const result = service.calculateForPosition(
      position,
      [stakingSource, feesSource, incentivesSource],
      calcDate
    );
...
describe("YieldService.processBatch", () => {
  it("processes all positions and returns results", async () => {
    ...
    const positions = Array.from({ length: 5 }, (_, i) =>
      makePosition({ id: `pos-${i}`, amount: 1_000 + i * 100 })
    );
  ...
  it("processes 1000 positions within a reasonable time (perf smoke test)", async () => {
    ...
    const positions = Array.from({ length: 1_000 }, (_, i) =>
      makePosition({ id: `p-${i}`, amount: 1_000 + i })
    );
...
describe("YieldScheduler", () => {
  ...
  it("calls onRunError when loader throws", async () => {
    ...
    expect((onRunError.mock.calls[0][0] as Error).message).toMatch(/DB connection lost/);
  });
